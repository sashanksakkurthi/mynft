...
def greet(num_images):
    img = generate(round(num_images))
    imgs = list(img)
    if len(imgs) == 1:
        return imgs[0]
    ...
    grid = np.zeros(
        (grid_height * imgs[0].shape[0], grid_len * imgs[0].shape[1], 3), dtype=np.uint8)
...
iface = gr.Interface(fn=greet, inputs=[
    gr.inputs.Slider(default=1, label="Num Images",
